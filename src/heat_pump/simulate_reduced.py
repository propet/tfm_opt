...
def get_dfdp(y, y_prev, p, u_prev, h):
    ...
    dfdp[0, 0] = (
        p[1] * ((y[0] - y_prev[0]) / h)
        - u_prev[1] * y[1]
        - u_prev[2] * (y[0] - p[5] * (y[0] - p[4]))
        + (u_prev[1] + u_prev[2]) * y[0]
    )
    dfdp[0, 1] = p[0] * ((y[0] - y_prev[0]) / h)
    dfdp[0, 2] = p[3] * (y[0] - p[4])
    dfdp[0, 3] = p[2] * (y[0] - p[4])
    dfdp[0, 4] = -u_prev[2] * p[0] * p[5] - p[2] * p[3]
    dfdp[0, 5] = u_prev[2] * p[0] * (y[0] - p[4])
    return dfdp
...
def get_dfdu(y, y_prev, p, u_prev, h):
    ...
    dfdu[0, 2] = -p[0] * (y[0] - p[5] * (y[0] - p[4])) + p[0] * y[0]
    return dfdu
...
def get_dfdy(y, y_prev, p, u_prev, h):
    ...
    dfdy = np.zeros((1, 2))
    dfdy[0, 0] = (
        (p[1] * p[0] / h) - u_prev[2] * p[0] + u_prev[2] * p[0] * p[5] + (u_prev[1] + u_prev[2]) * p[0] + p[2] * p[3]
    )
