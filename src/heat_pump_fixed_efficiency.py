...
from utils import get_solar_field_powers, get_grid_prices_kwh, get_electric_demand_powers, generic_plot, sigmoid
...
def objfunc_ks(grid_prices_kwh, p_gen, p_electric_demand):
    ...
    def return_function(xdict):
        ...
        for h in range(1, PARAMS["N_HOURS"] + 1):
            battery_soc.append(
                (PARAMS["SOC_MIN"] * PARAMS["MAX_BAT_CAPACITY"] + np.sum(eta[:h] * p_bat[:h]))
...
def run_optimization(plot=True):
    ...
    if plot:
        ...
        for h in range(1, PARAMS["N_HOURS"] + 1):
            battery_soc.append(
                (PARAMS["SOC_MIN"] * PARAMS["MAX_BAT_CAPACITY"] + np.sum(eta[:h] * sol.xStar["p_bat"][:h]))
                / PARAMS["MAX_BAT_CAPACITY"]
            )
...
if __name__ == "__main__":
    ...
    # #######################################################
    # cases = np.array(list(range(30, 140)))
    # obj_values = np.zeros_like(cases)
    # for i, q in enumerate(cases):
    #     print(f"\rProgress: {i}/{len(cases) - 1}", end="")
