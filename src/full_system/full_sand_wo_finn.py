...
def obj(opt, design_variables: DesignVariables) -> np.ndarray:
    ...
    cost = cost_function(h, cost_grid, p_bat, p_compressor, p_required, p_solar)
    return np.array(cost)


def cost_function(h, cost_grid, p_bat, p_compressor, p_required, p_solar):
...
def sens(opt, design_variables: DesignVariables, func_values):
    ...
    # p_grid = -p_solar + p_compressor + p_bat + p_required
    # cost = jnp.sum(h * cost_grid * p_grid)
    dcostdp_compressor = get_dcostdp_compressor(h, cost_grid, p_bat, p_compressor, p_required, p_solar)
...
def run_optimization(parameters, plot=True):
    opt = Opt("full_sand_wo_finn", obj, historyFileName="saves/full_sand_wo_finn.hst")
