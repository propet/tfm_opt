import numpy as np
from pyoptsparse import SLSQP, IPOPT
from opt import Opt
...
def run_optimization(plot=True):
    ...
    # # Solve
    # optOptions = {"IPRINT": -1}
    # slsqp = SLSQP(options=optOptions)
    # sol = slsqp(opt.optProb, sens="FD", sensStep=1e-6)
    ...
    ipopt = IPOPT(options=optOptions)
    sol = ipopt(opt.optProb, sens="FD", sensStep=1e-6)
